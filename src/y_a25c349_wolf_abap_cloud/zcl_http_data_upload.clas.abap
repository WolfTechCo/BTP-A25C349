CLASS zcl_http_data_upload DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS zcl_http_data_upload IMPLEMENTATION.
...
* Check table name is valid.
        IF NOT xco_cp_abap_repository=>object->tabl->database_table->for(
            iv_name = CONV #( tablename ) )->exists( )
         OR tablename IS INITIAL.
          response->set_status(
            i_code = if_web_http_status=>bad_request
